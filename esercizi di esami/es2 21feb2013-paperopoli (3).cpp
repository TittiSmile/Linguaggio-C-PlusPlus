...
int main ()
{
  ...
  x=min_pazienti(&r1,&m1,&p1,a1);
   cout << "Il reparto con meno pazienti e' "<<x<< endl;
...
void acquisisci (int r,int m,int p,int a[][M][R])
{
    ...
    
    {
        for (int j=0;j<m;j++)
        {
            for(int k=0;k<p;k++)
            {
                cout << "inserisci il numero giorni degenza per il reparto "<<i+1<<"\ndove lavora il medico "<<j+1<<"\nche cura il paziente "<<k+1<< endl;
                cin >> a[i][j][k];
...
void stampa_ospedale (int r,int m,int p,int a[][M][R])
{
    ...
    for (int i=0;i<r;i++)
    {
        cout<<endl<<"_______REPARTO "<<i+1<<"_________________"<<endl;
        for (int j=0;j<m;j++)
        {
            for(int k=0;k<p;k++)
            {
               cout << "Giorni rimasti al paziente  "<<k+1<<"  del medico "<<j+1<< ":"<< a[i][j][k]<< endl;     
            }
        }
     }   
...
int min_pazienti (int *r,int *m,int *p,int a[][M][R])
{
    ...
    for (int i=0;i<*r;i++)
    {
        ...
        somma=0; //reset conteggio pazienti per reparti...
    }
    return RepMin;
}
...
void pazienti (int *r,int *m,int *p,int a[][M][R])
{
    ...
    for (int i=0;i<*r;i++)
    {
        for (int j=0;j<*m;j++)
        {
         ...
         cout<<"nel reparto "<<i<<" sono ricoverati "<< temp <<" pazienti"<< endl;
               temp=0;
    }
