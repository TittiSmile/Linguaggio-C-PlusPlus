...
}while (base<2);  // "base<2" perchè si considera una base che sia almeno maggiore di 2 poichè 1 e 0 danno come risultato (indipendentemente dall'esponente) rispettivamente 1 e 0.
...
}while (esponente <0); // in questo caso,si presuppone che l'esponente sia maggiore di 0.
if (esponente==0)
{
    cout << "1" << endl; 
}else if (esponente==1)
